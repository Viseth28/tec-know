    ...
    <div className="space-y-6">
      <div className="flex flex-col md:flex-row gap-4 items-center justify-between">
        ...
        <div className="flex items-center space-x-2 w-full md:w-auto">
          ...
          <select
            className="flex-1 md:flex-none px-4 py-2 bg-white border border-gray-200 rounded-xl focus:ring-2 focus:ring-blue-500 outline-none"
          ...
          <table className="w-full text-left table-auto">
            <thead className="bg-gray-50 border-b border-gray-100">
              <tr className="text-[10px] text-gray-500 uppercase font-black tracking-widest">
                <th className="px-6 py-4 min-w-[200px]">Hardware Profile</th>
                <th className="px-6 py-4 w-[160px]">IMEI 1</th>
                <th className="px-6 py-4 w-[160px]">IMEI 2</th>
                <th className="px-6 py-4 w-[110px]">Condition</th>
                <th className="px-6 py-4 w-[130px]">Status</th>
                <th className="px-6 py-4 min-w-[150px]">Currently With</th>
                ...
                <tr key={phone.id} className="hover:bg-gray-50 transition-colors group">
                  <td className="px-6 py-4">
                    <div className="flex items-center">
                      <div className="p-2.5 bg-slate-50 border border-slate-100 rounded-xl mr-4 text-slate-400 group-hover:text-blue-500 transition-colors shrink-0">
                        <Smartphone size={20} />
                      </div>
                      <div className="truncate">
                        <p className="text-base font-black text-slate-900 leading-tight mb-0.5 truncate">{phone.model}</p>
                  ...
                  <td className="px-6 py-4">
                    <div className="bg-slate-50 px-2 py-1 rounded-lg border border-slate-100 inline-block">
                      <p className="text-[11px] font-mono font-black text-slate-700 tracking-normal">{phone.imei1}</p>
                    </div>
                  </td>
                  <td className="px-6 py-4">
                    <div className="bg-slate-50/50 px-2 py-1 rounded-lg border border-slate-100/50 inline-block">
                      <p className="text-[11px] font-mono font-bold text-slate-500 tracking-normal">{phone.imei2}</p>
                    </div>
                  </td>
                  <td className="px-6 py-4">
                    <span className={`text-xs font-black uppercase tracking-wider ${getConditionColor(phone.condition)}`}>
                      {phone.condition}
                    </span>
                  </td>
                  <td className="px-6 py-4">
                    <span className={`px-2.5 py-1.5 rounded-lg text-[10px] font-black uppercase tracking-widest ${getStatusColor(phone.status)} whitespace-nowrap`}>
                      {phone.status}
                    </span>
                  </td>
                  <td className="px-6 py-4">
                    {phone.borrower ? (
                      <div className="flex items-center space-x-2">
                        <div className="w-7 h-7 bg-blue-50 text-blue-600 rounded-full flex items-center justify-center text-[10px] font-black shrink-0">
                          {phone.borrower.name.charAt(0)}
                        </div>
                        <div className="truncate">
                          <p className="text-xs font-bold text-slate-900 truncate">{phone.borrower.name}</p>
                          <p className="text-[10px] text-slate-400 uppercase font-bold tracking-tighter truncate">{phone.borrower.department}</p>
                        </div>
                      </div>
                    ) : (
                    ...
                    <div className="flex items-center justify-end space-x-1">
                      {phone.status === DeviceStatus.AVAILABLE ? (
                        ...
                        <button
                          ...
                          className="p-2 hover:bg-green-50 text-slate-400 hover:text-green-600 rounded-xl transition-all"
                          title="Return"
                        >
                ...
                <tr>
                  <td colSpan={7} className="px-6 py-20 text-center">
                    <div className="flex flex-col items-center opacity-20">
                      ...
                      <p className="text-slate-900 font-black uppercase tracking-widest text-sm">No Hardware Found</p>
                    </div>
                  </td>
                </tr>
              )}
      ...
      {editingPhone && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4 animate-in fade-in duration-200">
          <div className="bg-white w-full max-w-lg rounded-[2.5rem] shadow-2xl overflow-y-auto max-h-[90vh] relative animate-in zoom-in-95 duration-300">
            <div className="sticky top-0 bg-white p-8 border-b border-gray-100 flex items-center justify-between z-10">
            ...
            <form onSubmit={handleUpdate} className="p-8 space-y-6">
              ...
              <div className="space-y-1">
                ...
                <input
                  required
                  maxLength={15}
                  className="w-full px-4 py-2 bg-gray-50 border rounded-xl font-mono focus:ring-2 focus:ring-blue-500 outline-none"
      ...
      {deletingId && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4 animate-in fade-in duration-200">
          <div className="bg-white w-full max-w-sm rounded-[2rem] shadow-2xl p-8 text-center animate-in zoom-in-95 duration-300">
            ...
            <p className="text-gray-500 mb-8">This action will permanently remove this device from your inventory system.</p>
